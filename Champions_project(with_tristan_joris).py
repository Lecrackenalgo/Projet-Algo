...
def init_bateau():
    ...
    for loop in range(int(nb_bat)):
        taille_bateau = input(
            f"Quelle taille pour le bateau numéro {loop+1} (entre 2 et 5) :  ")
...
def verif_placmnt_bateaux(c1, c2, n, grille_pos):
    ...
    elif grille_pos[c2[0]][c2[1]]:
        print("Vous essayez de placer un bateau trop proche d'un autre")
        res  = False
    else:
        print(c1, c2)
...
def placement_bateaux(n, grille_pos):
    ...
    if a != '10':  # 10 est le seul cas où on ne peut pas utiliser la fonction ord car '10' fait 2 caractères donc on traite ce cas à part
        while len(a) != 1 or ord(a) < 47 or ord(a) > 58 :
            print("Rentrez un chiffre entre 1 et 10 !", a)
            a = input("coordonnée ligne entre 1 et 10 :  ")
    a = int(a)
...
def trad_coordonnees(coordonnees):
    ...
    nouvelles_coordonnees.append(
        int(coordonnees[1:]))  # Position sur les lignes
    # Position sur les colonnes
    nouvelles_coordonnees.append(ord(coordonnees[0]) - 64)
        ...
        if res:
            if ligne <= 10 and ligne > 0:
                if ord(coord[0]) >= 65 and ord(coord[0]) <= 74:
                    ...
                    print(
                        "La coordonnée correspondant à la colonne doit être en majuscule entre A et J (inclus).\n", ex)
                    res = False
            else:
                print(
                    "Vous avez rentré un numéro de ligne qui ne rentre pas dans le tableau.\n", ex)
                res = False
    else:
...
def cherhcher_bateau(coord, grille_pos):
    nb_vie = 0
    #Horizontal
    pass
    #Vertical
...
grilles_jeu = [grille_jeu_1, grille_jeu_2]
...
# Initialisation des grilles de position
grille_position_1 = grille_pos(11, False)
# Initialisation des grilles de position
grille_position_2 = grille_pos(11, False)
grille_pos = [grille_position_1, grille_position_2]

if nombre_joueurs == 2:  # Placement des bateaux joueur 1 (pas tès optimisé)
    for i in range(2):
        for x in bateaux[i]:
            print(
                f"Le joueur {i+1} va placer ses bateaux. Joueur {2-i} fermez les yeux pour par de triche ;-)")
            placement_bateaux(x, grille_pos[i])
            affichage_bateaux(grille_pos[i])
    while somme_1 != 0 or somme_2 != 0:
        if tour % 2 == 0:
            print("C'est au tour de joueur 1\n")
        else:
            print("C'est au tour de joueur 2\n")
        afficher_grille(grilles_jeu[tour%2])
        ...
        bateau_touche(trad_coord, grille_pos[tour%2], grilles_jeu[tour%2], tour)
        somme_1 = sum(bateaux_1)
        somme_2 = sum(bateaux_2)
        tour += 1
